...
| **autonomous-chain-control-flow** | Wake-signal handling for long autonomous chains — a new user signal means *advance*, not restart; a step-0 state sweep re-establishes ground truth before acting. | Advance-vs-restart disambiguation, step-0 environment/state sweep, heartbeat-vs-GO distinction, idle-expire discipline |
| **claude-decision-discipline** | Five guards against model biases that override user-aligned principles — fired before proposals, recommendations, citations, and plan-skipping. | Anti-sycophancy check, evidence-before-causation gate, no motivated-reasoning scope-switch, citation-grounding, plan-before-skip |
| **codex-dispatch-watchdog** | 3-layer silent-stall defense for any `codex` CLI dispatch (pairs with the `tooling/` scripts below). | Internal log-inactivity watchdog, external session probe (cron), sentinel-freshness PreToolUse hook, `stdin=/dev/null` forcing |
| **terminology-audit** | Cross-file term-drift audit + safe corpus-wide unification with count and build verification. | Separate-pass summary-table vs prose scan, occurrence counting, post-unify build/compile check, forbidden-term mapping |
| **user-explanation-5step** | Compact decision-brief shape for inline trade-off / verdict replies. | 问题定义 → 公式/表 → 结论 structure, evidence-strength labels, tool-name / file-path bleed block |
| **write-research-readout** *(project-scoped)* | Produce presentation-grade markdown readouts for research deliverables — formula-first, symbol-tabled, artifact-cited prose with side-by-side `(c) 独立复现` columns and axis-structured amendments. Never function-name dumps or code blocks for what should be math. | 13 style fingerprints derived from a canonical group-meeting deck exemplar: 符号表 → 公式化方法 → named acceptance criteria with multi-version status columns → Defect-history blockquotes with mathematical falsification → sub-step recipes with anti-confusion annotations → math-incompatibility proofs for retracted gates → axis-structured amendments with N-way sub-frame categorization → ASCII timeline + 顺序路径 blocks → comparability-neutral vs 已主动避开 risk pairing → Claim-evolution comparison tables → preempt-actual-critique Q&A → two-block 附录 with commit + artifact + ASCII schema. **The mandatory `(c) 独立复现` column on every empirical number writes dual-review boundary (c) into the readout itself**, so a paper-facing claim cannot enter a deck without the second-reviewer rerun number alongside. **Reusable on other projects only after substituting the workspace-specific harness rules and exemplar references.** |
...
## Codex dispatch tooling (`tooling/`)

The dispatch toolchain the `codex-orchestration` + `codex-dispatch-watchdog` skills assume. Drop into your project (e.g. `scripts/` + `scripts/hooks/`) and wire the guards via `settings.hooks.example.json`.

| File | Role |
|------|------|
| `dispatch_codex.sh` | `codex exec` wrapper with a stall-detection watchdog — forces `stdin=/dev/null`, kills on log-inactivity, checks the deliverable exists. |
| `codex_probe_heartbeat_tick.sh` | Probe tick: refreshes a sentinel while a dispatch is active, self-expires after 2 idle ticks. |
| `hooks/codex_exec_guard.sh` | Blocks raw `codex exec` that bypasses the wrapper. |
| `hooks/codex_probe_guard.sh` | Blocks dispatch unless an active probe sentinel is fresh (≤270s). |
| `hooks/codex_dispatch_pattern_guard.sh` | Blocks `&` / `nohup` / `disown` double-backgrounding that hides the wrapper from the harness. |
| `settings.hooks.example.json` | Example Claude Code `settings.json` wiring for the three `PreToolUse:Bash` guards. |

> These scripts assume the [`codex`](https://github.com/openai/codex) CLI on PATH. Adjust the model id / flags to yours.
...
**Project-scoped skills** (currently: `write-research-readout`) are kept in this collection for provenance and cross-machine portability, but they encode the conventions of one specific research workspace. To reuse on a different project, fork the SKILL.md and substitute (a) the project-frame harness rules referenced in its body, (b) the canonical exemplar paths (`presentation_4.27/...`, `presentation_4.23/...`), and (c) the deliverable file-location convention. The 13 style fingerprints themselves are project-agnostic and transfer directly.
