#!/usr/bin/env python3
"""Rust refactor: split by item boundaries or chunk size."""
...
def split_by_patterns(lines, patterns):
    sections = []
    current = []
    for line in lines:
        stripped = line.strip()
        if any(stripped.startswith(pat) for pat in patterns) and current:
...
def write_chunks(src: Path, output_dir: Path, chunks):
    ...
    for idx, chunk in enumerate(chunks, 1):
        part_path = output_dir / f"{src.stem}_part{idx}.rs"
        part_path.write_text("\n".join(chunk) + "\n", encoding="utf-8")


def main():
    parser = argparse.ArgumentParser(description="Rust modularizer")
    parser.add_argument("file_path", help="Rust source file")
    ...
    patterns = ["pub struct", "struct ", "enum ", "impl ", "fn "]
    sections = split_by_patterns(lines, patterns)
    if len(sections) == 1:
